...
const ModalCreateSlide = ({ openModal, setOpenModal, fetchData }) => {
  ...
  const handleSubmit = async () => {
    if (!fileName) {
      ...
      const { data, error } = await supabaseAdmin.from("slides").insert({
        url: `https://weeevlktjgkhinnqsmai.supabase.co/storage/v1/object/public/post_images/slides/${fileName}`,
      ...
      if (error) {
        setImage(null);
        toast.error(`Thêm mới thất bại, lỗi ${error.message}`);
      }
      setOpenModal(false);
      setFileName(null);
    }
  ...
  return (
    ...
    >
      <Modal.Header>Thêm mới slide</Modal.Header>
      <Modal.Body>
